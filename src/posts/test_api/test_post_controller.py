...
def test_create_post(client):
    ...
    with open(IMAGE_PATH, "rb") as file:
        ...
        response = requests.post(f"{client}/", params=post_data, json=post_data, files=files)

    # Check response
    assert response.status_code == status.HTTP_200_OK, f"Failed to create post: {response.json()}"
    post = response.json()
...
def test_get_deleted_post(client):
    ...
    response = requests.get(f"{client}/{TEST_POST_ID}")
    assert response.status_code == status.HTTP_404_NOT_FOUND, "Deleted post still retrievable"
