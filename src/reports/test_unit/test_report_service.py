...
def test_create_report(report_service, sample_report, db_connection):
    ...
    cursor.execute("SELECT * FROM Report WHERE ReportID = ?", (sample_report.report_id,))
...
def test_update_report_status(report_service, sample_report):
    ...
    result_code = report_service.update_report_status(sample_report.report_id, new_status)
    assert result_code == 200

    # Verify the status update
    cursor = report_service.conn.cursor()
    cursor.execute("SELECT Status FROM Report WHERE ReportID = ?", (sample_report.report_id,))
    result = cursor.fetchone()
...
def test_update_report_points(report_service, sample_report):
    ...
    # Update points
    new_points = 50
    result_code = report_service.update_report_points(sample_report.report_id, new_points)
    assert result_code == 200

    # Verify the points update
    cursor = report_service.conn.cursor()
    cursor.execute("SELECT Points FROM Report WHERE ReportID = ?", (sample_report.report_id,))
