...
def test_update_reward(reward_service, sample_reward):
    ...
    cursor.execute("SELECT Description, PointsRequired, Validity, Availability FROM Reward WHERE RewardID = ?", (sample_reward.rewardID,))
    result = cursor.fetchone()
    assert result == (reward_update.description, reward_update.pointsRequired, reward_update.validity, reward_update.availability)
...
def test_claim_reward_by_id(reward_service, sample_reward, db_connection):
    ...
    # Claim the reward
    status_code, reward = reward_service.claim_reward_by_id(sample_reward.rewardID, user_id)
    assert status_code == 200
...
def test_update_reward_availability(reward_service, sample_reward):
    ...
    # Update availability
    new_availability = 20
    status_code = reward_service.update_reward_availability(sample_reward.rewardID, new_availability)
    assert status_code == 200

    # Verify the update
    cursor = reward_service.conn.cursor()
    cursor.execute("SELECT Availability FROM Reward WHERE RewardID = ?", (sample_reward.rewardID,))
    result = cursor.fetchone()
