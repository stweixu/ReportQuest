...
def test_create_reward(client):
    ...
    assert reward["availability"] == reward_data["availability"], "Availability mismatch"
...
def test_update_reward_quantity(client):
    ...
    response = requests.put(f"{client}/{TEST_REWARD_ID}/quantity", json={"quantity": 20})
    if response.status_code == status.HTTP_200_OK:
...
def test_generate_encoded_reward(client):
    ...
    response = requests.post(f"{client}/{TEST_REWARD_ID}/generate", json={"duration": 30})
    if response.status_code == status.HTTP_200_OK:
...
def test_claim_reward(client):
    ...
    user_id = "sample_user"
    response = requests.post(f"{client}/claim/{TEST_REWARD_ID}", json={"user_id": user_id})
    if response.status_code == status.HTTP_200_OK:
